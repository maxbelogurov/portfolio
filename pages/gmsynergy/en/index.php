<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta name="theme-color" content="#818181">
	<meta name="yandex-verification" content="b174213608f5fca3" />
	<meta name="google-site-verification" content="ECQPE3hYSJJAG8OA1dfqUhJoo-rs2ZXVKxV1nMQ3BkE" />
	<title>Virtual Mining</title>
    <meta name="description" content="Platform, where anybody could try yourself as a miner and earn real currency without buying equipment.">
	
    <link rel="icon" sizes="192x192" href="../favicon.png" type="image/png">
	<?php echo "<style>";
		include("../css/bootstrap.min.css");
		include("../css/font-awesome.min.css");
		include("../css/style.css");
	echo "</style>"; ?>
	<link rel="canonical" href="https://vmtoken.io/en/" hreflang="en">
	<link rel="alternate" hreflang="en">
  </head>
  <body>
	
	<nav class="navbar navbar-expand navbar-light fixed-top" id="mainNav">
      <div class="container">
        <div class="collapse navbar-collapse" id="navbarResponsive">
          <ul class="navbar-nav ml-auto">
            <li class="nav-item">
              <a class="nav-link language" href="/">RUS</a>
            </li>
          </ul>
        </div>
      </div>
    </nav>
  
    <header class="container-fluid start">
		<div class="row text-center justify-content-center logo">
			<div class="col-12">	
				<img class="img-fluid" style="width:200px;" src="../logo.png" alt="Logotype Virtual Mining">
				<h1 class="red font-weight-normal">Virtual Mining</h1>
				<h4 class="grey">You play we pay</h4>
			</div>
		</div>
		<div class="row text-center justify-content-md-center no-gutters">
			<div class="d-none d-lg-block col-sm-12 ">	
				<a class="scroll-icon" href="#game"><i class="fa fa-sort-desc fa-fw fa-3x"></i></a>
			</div>
		</div>
	</header>
	
	<div id="game" class="container-fluid">
		<div class="row text-center justify-content-center no-gutters">
			<div class="col-11 col-sm-7">
				<span style="font-size:1.75rem;display:block;" class="mb-5">Idea of Virtual Mining</span>
				<p>Everybody who could try yourself as a miner and in a time to get real value without buying expensive equipment, setting it up and supporting it. All this we present you as a comfortable game environment in usual browser, which can provide you to monitor the status of your equipment, difficulties of common network, electricity consumption and getting profit.</p>
				<p class="mb-5">Owing to virtual equipment it is possible to mine BTC and other top crypto-current with minimal start attachment.</p>
			</div>
		</div>
		<div class="row text-center justify-content-center no-gutters">
			<div class="col-11 col-md-4 col-sm-7">
				<div class="embed-responsive embed-responsive-16by9">
					<iframe class="embed-responsive-item" src="https://www.youtube.com/embed/WiHgix4NiFo?rel=0"></iframe>
				</div>
				<p class="mb-5"></p>
			</div>
		</div>		
		<div class="row justify-content-center no-gutters game-icons">
			<div class="col-3 col-xl-1 mt-4 text-center">
				<span class="svg-icons" data-toggle="tooltip" data-placement="bottom" title="Easy to start: build your own mining farm in virtual reality with minimum knowledge and attachment.">
				<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
				viewBox="0 0 425.2 425.2" style="enable-background:new 0 0 425.2 425.2;" xml:space="preserve">
				<g>
				<path d="M218.407,88.8h12c4.4,0,8-3.6,8-8c0-4.4-3.6-8-8-8h-12c-4.4,0-8,3.6-8,8C210.407,85.2,214.007,88.8,218.407,88.8z"/>
				<path d="M210.407,152.8c-22,0-40,18-40,40s18,40,40,40s40-18,40-40S232.407,152.8,210.407,152.8z M210.407,216.8
					c-13.2,0-24-10.8-24-24s10.8-24,24-24s24,10.8,24,24S223.607,216.8,210.407,216.8z"/>
				<path d="M354.807,415.6l-12-65.6c-3.2-17.2-13.6-31.2-28.4-39.2l-22.8-12.4c7.2-27.2,11.2-55.6,11.2-84v-10.8
					c0-60.8-18.8-118.8-54.4-168.8l-18-24.8c-4.4-6.4-11.6-10-19.6-10c-7.6,0-14.8,3.6-19.6,10l-18,24.8
					c-35.2,49.2-54.8,109.2-54.8,169.2v10.8c0,28.8,4,57.6,11.6,85.6l-19.2,10.4c-14.8,8-25.2,22-28.4,39.2l-12,65.2
					c-0.4,3.2,0.8,6.8,4,8.4c1.2,0.8,2.8,1.2,4,1.2c2,0,3.6-0.8,5.2-2l33.6-28.8c12.8-11.2,29.6-17.2,46.8-17.2h14.4
					c0.4,0,0.8,0,1.2,0c0.8,0,2,0,2.8,0h8v26c0,12,10,22,22,22s22-10,22-22v-26.4h4h0.4c1.2,0.4,2.4,0.8,3.6,0.8h18.8
					c17.2,0,34,6,46.8,17.2l33.6,28.8c1.6,1.2,3.2,2,5.2,2c1.2,0,2.8-0.4,4-1.2C353.607,422.4,355.207,419.2,354.807,415.6z
					 M106.807,382l-16.4,14.4l7.6-43.2c2.4-12.4,9.6-22.4,20.4-28l16-8.8l9.6,32c1.6,5.2,4,9.6,7.2,13.6
					C134.807,364.4,119.207,371.2,106.807,382z M218.407,402.8c0,3.2-2.8,6-6,6s-6-2.8-6-6v-84c0-3.2,2.8-6,6-6s6,2.8,6,6V402.8z
					 M238.407,360.4h-4v-41.6c0-12-10-22-22-22s-22,10-22,22v42h-8c-10.8,0-20-7.2-23.2-17.2l-12-40.4c-8.4-28.8-12.8-58.4-12.8-88.4
					V204c0-39.6,8.8-79.2,25.6-115.2h30.4c4.4,0,8-3.6,8-8c0-4.4-3.6-8-8-8h-22c5.2-10,11.2-19.2,18-28.4l18-24.8
					c1.6-2,4-3.2,6.4-3.2c2.8,0,4.8,1.2,6.4,3.2l18,24.8c33.6,46.8,51.2,102,51.2,159.2v10.8c0,30-4.4,60-12.8,88.4l-12,40.4
					C258.407,353.6,249.207,360.4,238.407,360.4z M318.407,382.4c-13.6-11.6-30.8-18.8-48.8-20.8c3.2-4,5.6-8.4,7.2-13.6l10-33.6
					l19.6,10.8c10.4,5.6,18,16,20.4,28l8,43.2L318.407,382.4z"/>
				</g>
				</svg>	
				</span>
			</div>
			
			<div class="col-3 col-xl-1 mt-4 text-center">
				<span class="svg-icons" data-toggle="tooltip" data-placement="bottom" title="Web-interface: play any time and from any point of the Earth.">
				<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 480.1 480.1" style="enable-background:new 0 0 480.1 480.1;" xml:space="preserve">
				<g>
					<path d="M240.135,0.05C144.085,0.036,57.277,57.289,19.472,145.586l-2.992,0.992l1.16,3.48
					c-49.776,122.766,9.393,262.639,132.159,312.415c28.673,11.626,59.324,17.594,90.265,17.577
					c132.548,0.02,240.016-107.416,240.036-239.964S372.684,0.069,240.135,0.05z M428.388,361.054l-12.324-12.316V320.05
					c0.014-1.238-0.26-2.462-0.8-3.576l-31.2-62.312V224.05c0-2.674-1.335-5.172-3.56-6.656l-24-16
					c-1.881-1.256-4.206-1.657-6.4-1.104l-29.392,7.344l-49.368-21.184l-6.792-47.584l18.824-18.816h40.408l13.6,20.44
					c1.228,1.838,3.163,3.087,5.344,3.448l48,8c1.286,0.216,2.604,0.111,3.84-0.304l44.208-14.736
					C475.855,208.053,471.889,293.634,428.388,361.054z M395.392,78.882l-13.008,8.672l-36.264-7.256l-23.528-7.832
					c-1.44-0.489-2.99-0.551-4.464-0.176l-29.744,7.432l-13.04-4.344l9.664-19.328h27.056c1.241,0.001,2.465-0.286,3.576-0.84
					l27.68-13.84C362.382,51.32,379.918,63.952,395.392,78.882z M152.44,33.914l19.2,12.8c0.944,0.628,2.01,1.048,3.128,1.232
					l38.768,6.464l-3.784,11.32l-20.2,6.744c-1.809,0.602-3.344,1.83-4.328,3.464l-22.976,38.288l-36.904,22.144l-54.4,7.768
					c-3.943,0.557-6.875,3.93-6.88,7.912v24c0,2.122,0.844,4.156,2.344,5.656l13.656,13.656v13.744l-33.28-22.192l-12.072-36.216
					C57.68,98.218,99.777,56.458,152.44,33.914z M129.664,296.21l-36.16-7.24l-13.44-26.808v-18.8l29.808-29.808l11.032,22.072
					c1.355,2.712,4.128,4.425,7.16,4.424h51.472l21.672,36.12c1.446,2.407,4.048,3.879,6.856,3.88h22.24l-5.6,28.056l-30.288,30.288
					c-1.503,1.499-2.349,3.533-2.352,5.656v20l-28.8,21.6c-2.014,1.511-3.2,3.882-3.2,6.4v28.896l-9.952-3.296l-14.048-35.136V304.05
					C136.065,300.248,133.389,296.97,129.664,296.21z M105.616,419.191C30.187,362.602-1.712,264.826,25.832,174.642l6.648,19.936
					c0.56,1.687,1.666,3.14,3.144,4.128l39.88,26.584l-9.096,9.104c-1.5,1.5-2.344,3.534-2.344,5.656v24
					c-0.001,1.241,0.286,2.465,0.84,3.576l16,32c1.108,2.21,3.175,3.784,5.6,4.264l33.6,6.712v73.448
					c-0.001,1.016,0.192,2.024,0.568,2.968l16,40c0.876,2.185,2.67,3.874,4.904,4.616l24,8c0.802,0.272,1.642,0.412,2.488,0.416
					c4.418,0,8-3.582,8-8v-36l28.8-21.6c2.014-1.511,3.2-3.882,3.2-6.4v-20.688l29.656-29.656c1.115-1.117,1.875-2.54,2.184-4.088
					l8-40c0.866-4.333-1.944-8.547-6.277-9.413c-0.515-0.103-1.038-0.155-1.563-0.155h-27.472l-21.672-36.12
					c-1.446-2.407-4.048-3.879-6.856-3.88h-51.056l-13.744-27.576c-1.151-2.302-3.339-3.91-5.88-4.32
					c-2.54-0.439-5.133,0.399-6.936,2.24l-10.384,10.344V192.05c0-2.122-0.844-4.156-2.344-5.656l-13.656-13.656v-13.752l49.136-7.016
					c1.055-0.153,2.07-0.515,2.984-1.064l40-24c1.122-0.674,2.062-1.614,2.736-2.736l22.48-37.464l21.192-7.072
					c2.393-0.785,4.271-2.662,5.056-5.056l8-24c1.386-4.195-0.891-8.72-5.086-10.106c-0.387-0.128-0.784-0.226-1.186-0.294
					l-46.304-7.72l-8.136-5.424c50.343-16.386,104.869-14.358,153.856,5.72l-14.616,7.296h-30.112c-3.047-0.017-5.838,1.699-7.2,4.424
					l-16,32c-1.971,3.954-0.364,8.758,3.59,10.729c0.337,0.168,0.685,0.312,1.042,0.431l24,8c1.44,0.489,2.99,0.551,4.464,0.176
					l29.744-7.432l21.792,7.256c0.312,0.112,0.633,0.198,0.96,0.256l40,8c2.08,0.424,4.244-0.002,6.008-1.184l18.208-12.144
					c8.961,9.981,17.014,20.741,24.064,32.152l-39.36,13.12l-42.616-7.104l-14.08-21.12c-1.476-2.213-3.956-3.547-6.616-3.56h-48
					c-2.122,0-4.156,0.844-5.656,2.344l-24,24c-1.782,1.781-2.621,4.298-2.264,6.792l8,56c0.403,2.769,2.223,5.126,4.8,6.216l56,24
					c1.604,0.695,3.394,0.838,5.088,0.408l28.568-7.144l17.464,11.664v27.72c-0.014,1.238,0.26,2.462,0.8,3.576l31.2,62.312v30.112
					c0,2.122,0.844,4.156,2.344,5.656l16.736,16.744C344.921,473.383,204.549,493.415,105.616,419.191z"/>		
				</g>
				</svg>
				</span>
			</div>
			
			<div class="col-3 col-xl-1 mt-4 text-center">
				<span class="svg-icons" data-toggle="tooltip" data-placement="bottom" title="Game process: real payment for interesting gameplay.">
					<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
					 viewBox="0 0 321.145 321.145" style="enable-background:new 0 0 321.145 321.145;" xml:space="preserve">
				<g>
					<path d="M320.973,200.981c-0.8-18.4-4-38.8-8.8-58c-4.8-18.4-10.8-35.6-18-48.8c-28-49.2-58.4-41.6-94.8-32
						c-11.6,2.8-24,6-36.8,7.2h-4c-12.8-1.2-25.2-4.4-36.8-7.2c-36.4-9.2-66.8-17.2-94.8,32.4c-7.2,13.2-13.6,30.4-18,48.8
						c-4.8,19.2-8,39.6-8.8,58c-0.8,20.4,1.2,35.2,5.6,45.6c4.4,9.6,10.8,15.6,19.2,18c7.6,2,16.4,1.2,25.6-2.8
						c15.6-6.4,33.6-20,51.2-36.4c12.4-12,35.6-18,58.8-18s46.4,6,58.8,18c17.6,16.4,35.6,30,51.2,36.4c9.2,3.6,18,4.8,25.6,2.8
						c8-2.4,14.8-8,19.2-18.4C319.773,236.581,321.773,221.781,320.973,200.981z M301.773,240.981c-2.4,5.6-5.6,8.8-9.6,10
						c-4.4,1.2-10,0.4-16.4-2c-14-5.6-30.4-18-46.4-33.2c-15.2-15.2-42-22.8-68.8-22.8s-53.6,7.6-69.2,22c-16.4,15.2-32.8,28-46.4,33.2
						c-6.4,2.4-12,3.6-16.4,2c-4-1.2-7.2-4.4-9.6-10c-3.2-7.6-4.8-20-4-38.4c0.8-17.2,3.6-36.8,8.4-55.2c4.4-17.2,10-33.2,16.8-45.2
						c22-39.6,47.6-33.2,78-25.2c12.4,3.2,25.2,6.4,39.2,7.6c0.4,0,0.4,0,0.8,0h4.4c0.4,0,0.4,0,0.8,0c14.4-1.2,27.2-4.4,39.6-7.6
						c30.4-7.6,56-14.4,78,25.2c6.8,12,12.4,27.6,16.8,45.2c4.4,18.4,7.6,37.6,8.4,55.2
						C306.973,220.181,305.373,232.581,301.773,240.981z"/>
					<path d="M123.773,122.981c-4-3.6-8.8-6.4-14.4-6.8c-0.4-5.2-2.8-10.4-6.4-14l-0.4-0.4c-4.4-4.4-10-6.8-16.4-6.8
						c-6.4,0-12.4,2.8-16.4,6.8c-3.6,3.6-6.4,8.8-6.8,14.4c-5.6,0.4-10.4,2.8-14.4,6.4l-0.4,0.4c-4.4,4.4-6.8,10-6.8,16.4
						c0,6.4,2.8,12.4,6.8,16.4c4,4,8.8,6.4,14.8,6.8c0.4,5.6,2.8,10.8,6.8,14.4c4.4,4.4,10,6.8,16.4,6.8c6.4,0,12.4-2.8,16.4-6.8
						c3.6-4,6.4-8.8,6.8-14.4c5.6-0.4,10.8-2.8,14.4-6.8c4.4-4.4,6.8-10,6.8-16.4C130.573,132.981,127.773,126.981,123.773,122.981z
						 M113.773,145.381c-1.6,1.6-3.6,2.4-6,2.4h-5.6c-4,0-7.6,3.2-7.6,7.6v5.2c0,2.4-0.8,4.4-2.4,6c-1.6,1.6-3.6,2.4-6,2.4
						c-2.4,0-4.4-0.8-6-2.4c-1.6-1.6-2.4-3.6-2.4-6v-5.6c0-4-3.2-7.6-7.6-7.6h-5.6c-2.4,0-4.4-0.8-6-2.4c-1.2-1.2-2.4-3.2-2.4-5.6
						c0-2.4,0.8-4.4,2.4-6c0,0,0,0,0.4-0.4c1.6-1.2,3.6-2,5.6-2h5.6c4,0,7.6-3.2,7.6-7.6v-5.6c0-2.4,0.8-4.4,2.4-6
						c1.6-1.6,3.6-2.4,6-2.4c2.4,0,4.4,0.8,6,2.4c0,0,0,0,0.4,0.4c1.2,1.6,2,3.6,2,5.6v5.6c0,4,3.2,7.6,7.6,7.6h5.6
						c2.4,0,4.4,0.8,6,2.4c1.6,1.6,2.4,3.6,2.4,6C116.173,141.781,115.373,143.781,113.773,145.381z"/>
					<circle cx="230.173" cy="110.981" r="14"/>
					<circle cx="230.173" cy="167.781" r="14"/>
					<circle cx="201.773" cy="139.381" r="14"/>
					<circle cx="258.573" cy="139.381" r="14"/>					
				</g>
				</svg>
				</span>
			</div>
			
			<div class="col-3 col-xl-1 mt-4 text-center">
				<span class="svg-icons" data-toggle="tooltip" data-placement="bottom" title="Provision: become a part of real working mining system.">
				<svg version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 269.044 269.044" xmlns:xlink="http://www.w3.org/1999/xlink" enable-background="new 0 0 269.044 269.044">
				  <g>
					<path d="m220.522,167.599h-22.223l-17.039-89.786c-0.537-2.832-3.012-4.882-5.895-4.882h-25.75c-2.841,0-5.293,1.993-5.873,4.774l-13.607,65.189-9.612-50.654c-0.537-2.832-3.012-4.882-5.895-4.882h-21.825c-2.841,0-5.293,1.993-5.873,4.773l-15.753,75.466h-22.655c-3.313,0-6,2.687-6,6v89.333c0,3.313 2.687,6 6,6h52.77c0.372,0.072 0.754,0.112 1.146,0.112h64.168c0.392,0 0.775-0.041 1.146-0.112h52.77c3.313,0 6-2.687 6-6v-89.333c0-3.312-2.687-5.998-6-5.998zm-112.084,89.445v-21.66h52.168v21.66h-52.168zm106.084-.112h-41.916v-21.548h2.486c3.313,0 6-2.687 6-6s-2.687-6-6-6h-81.141c-3.313,0-6,2.687-6,6s2.687,6 6,6h2.487v21.548h-41.916v-77.333h160v77.333zm-104.862-157.573l12.95,68.239h-39.175l14.245-68.239h11.98zm60.737-14.427l15.688,82.667h-48.849l17.256-82.667h15.905z"/>
					<path d="m161.592,66c3.313,0 6-2.687 6-6 0-2.59 0.494-5.062 1.355-7.355 2.128-5.664 6.625-10.161 12.29-12.29 2.293-0.862 4.765-1.355 7.355-1.355 18.196,0 33-14.804 33-33 0-3.313-2.687-6-6-6s-6,2.687-6,6c0,2.591-0.494,5.063-1.356,7.356-2.128,5.663-6.625,10.159-12.288,12.288-2.293,0.862-4.766,1.356-7.356,1.356-18.196,0-33,14.804-33,33 2.84217e-14,3.313 2.687,6 6,6z"/>
					<path d="m102.955,73.396c3.313,0 6-2.687 6-6 0-2.591 0.494-5.063 1.356-7.356 2.128-5.663 6.625-10.16 12.289-12.289 2.293-0.862 4.765-1.356 7.356-1.356 18.196,0 33-14.804 33-33 0-3.313-2.687-6-6-6s-6,2.687-6,6c0,2.591-0.494,5.063-1.356,7.356-2.128,5.663-6.625,10.16-12.288,12.288-2.293,0.862-4.765,1.356-7.356,1.356-18.196,0-33,14.804-33,33-0.001,3.314 2.685,6.001 5.999,6.001z"/>
					<path d="m79.544,193.718h-8.614c-3.313,0-6,2.687-6,6s2.687,6 6,6h8.614c3.313,0 6-2.687 6-6s-2.686-6-6-6z"/>
					<path d="m109.186,193.718h-8.613c-3.313,0-6,2.687-6,6s2.687,6 6,6h8.613c3.313,0 6-2.687 6-6s-2.686-6-6-6z"/>
					<path d="m138.829,193.718h-8.614c-3.313,0-6,2.687-6,6s2.687,6 6,6h8.614c3.313,0 6-2.687 6-6s-2.687-6-6-6z"/>
					<path d="m168.471,193.718h-8.614c-3.313,0-6,2.687-6,6s2.687,6 6,6h8.614c3.313,0 6-2.687 6-6s-2.686-6-6-6z"/>
					<path d="m183.499,199.718c0,3.313 2.687,6 6,6h8.614c3.313,0 6-2.687 6-6s-2.687-6-6-6h-8.614c-3.314,0-6,2.686-6,6z"/>
				  </g>
				</svg>
				</span>
			</div>
			
			<div class="col-3 col-xl-1 mt-4 text-center">
				<span class="svg-icons" data-toggle="tooltip" data-placement="bottom" title="Security: get an unlimited term of contract and guarantee on your equipment.">
					<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
					viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
						<g>
						<path d="M230.792,354.313l-6.729,60.51c-0.333,3.01,0.635,6.031,2.656,8.292c2.021,2.26,4.917,3.552,7.948,3.552h42.667
							c3.031,0,5.927-1.292,7.948-3.552c2.021-2.26,2.99-5.281,2.656-8.292l-6.729-60.51c10.927-7.948,17.458-20.521,17.458-34.313
							c0-23.531-19.135-42.667-42.667-42.667S213.333,296.469,213.333,320C213.333,333.792,219.865,346.365,230.792,354.313z
							 M256,298.667c11.76,0,21.333,9.573,21.333,21.333c0,8.177-4.646,15.5-12.125,19.125c-4.073,1.979-6.458,6.292-5.958,10.781
							l6.167,55.427h-18.833l6.167-55.427c0.5-4.49-1.885-8.802-5.958-10.781c-7.479-3.625-12.125-10.948-12.125-19.125
							C234.667,308.24,244.24,298.667,256,298.667z"/>
						<path d="M437.333,192h-32v-42.667C405.333,66.99,338.344,0,256,0S106.667,66.99,106.667,149.333V192h-32
							C68.771,192,64,196.771,64,202.667v266.667C64,492.865,83.135,512,106.667,512h298.667C428.865,512,448,492.865,448,469.333
							V202.667C448,196.771,443.229,192,437.333,192z M128,149.333c0-70.583,57.417-128,128-128s128,57.417,128,128V192h-21.333
							v-42.667c0-58.813-47.854-106.667-106.667-106.667S149.333,90.521,149.333,149.333V192H128V149.333z M341.333,149.333V192
							H170.667v-42.667C170.667,102.281,208.948,64,256,64S341.333,102.281,341.333,149.333z M426.667,469.333
							c0,11.76-9.573,21.333-21.333,21.333H106.667c-11.76,0-21.333-9.573-21.333-21.333v-256h341.333V469.333z"/>
						</g>
					</svg>
				</span>
			</div>
			
			<div class="col-3 col-xl-1 mt-4 text-center">
				<span class="svg-icons" data-toggle="tooltip" data-placement="bottom" title="Control: transparent system of income and expense for flexible control of investments.">
					<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
					viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
					<g>
					<path d="M512,126.66c0-13.209-2.271-26.008-6.749-38.041c-11.125-30.65-35.083-54.6-65.671-65.69
						c-0.004-0.002-0.01-0.003-0.014-0.005c-0.005-0.002-0.012-0.004-0.017-0.006c-12.032-4.479-24.831-6.749-38.04-6.749H110.492
						c-29.41,0-57.128,11.521-78.05,32.444C11.521,69.531,0,97.25,0,126.659c0,60.926,49.566,110.493,110.492,110.493h291.016
						c13.221,0,26.03-2.275,38.005-6.736c0.009-0.003,0.017-0.006,0.026-0.01c0.025-0.01,0.05-0.019,0.075-0.028
						c30.617-11.136,54.543-35.08,65.626-65.646c0.002-0.006,0.004-0.014,0.008-0.022c0.002-0.006,0.005-0.013,0.008-0.019h-0.001
						C509.729,152.661,512,139.865,512,126.66z M110.492,220.957c-51.996,0-94.297-42.302-94.297-94.298
						c0-25.083,9.837-48.734,27.699-66.596s41.514-27.699,66.597-27.699h233.483c-31.737,19.435-52.959,54.434-52.959,94.297
						c0,39.864,21.221,74.861,52.959,94.297H110.492z M490.042,159.132c-4.68,12.765-12.061,24.247-21.937,34.122
						c-9.84,9.84-21.274,17.202-33.984,21.884c-0.066,0.024-0.13,0.05-0.195,0.073c-10.227,3.81-21.134,5.742-32.418,5.742
						c-51.996,0-94.297-42.301-94.297-94.297s42.301-94.297,94.297-94.297c11.277,0.001,22.178,1.932,32.466,5.761
						c26.055,9.589,46.494,30.03,56.077,56.087l0.012,0.031c3.81,10.228,5.742,21.135,5.742,32.419
						C495.805,137.939,493.874,148.842,490.042,159.132z"/>
					<path d="M401.509,274.848H110.492c-13.221,0-26.03,2.275-38.006,6.736c-0.009,0.003-0.017,0.006-0.026,0.01
						c-0.025,0.01-0.05,0.019-0.076,0.029c-30.617,11.136-54.542,35.08-65.625,65.646c-0.002,0.006-0.004,0.014-0.008,0.022
						c-0.002,0.006-0.005,0.013-0.008,0.019h0.001C2.271,359.339,0,372.134,0,385.34c0,13.208,2.271,26.006,6.748,38.037
						c11.125,30.65,35.084,54.603,65.672,65.694c0.004,0.002,0.01,0.003,0.014,0.005c0.005,0.002,0.012,0.004,0.017,0.007
						c12.033,4.479,24.831,6.749,38.041,6.749h291.016c29.41,0,57.127-11.521,78.049-32.444C500.478,442.467,512,414.749,512,385.34
						C512,324.414,462.434,274.848,401.509,274.848z M78.026,473.877c-26.055-9.589-46.494-30.03-56.077-56.088l-0.012-0.031
						c-3.81-10.227-5.742-21.134-5.742-32.418c0-11.281,1.93-22.183,5.762-32.474c4.68-12.765,12.061-24.247,21.937-34.124
						c9.84-9.84,21.274-17.202,33.984-21.884c0.066-0.024,0.13-0.05,0.195-0.073c10.228-3.81,21.135-5.742,32.419-5.742
						c51.996,0,94.297,42.301,94.297,94.297s-42.301,94.297-94.297,94.297C99.216,479.637,88.315,477.707,78.026,473.877z
						 M468.105,451.937c-17.862,17.862-41.514,27.699-66.597,27.699H168.025c31.737-19.435,52.959-54.434,52.959-94.297
						c0-39.864-21.222-74.861-52.959-94.297h233.484c51.996,0,94.297,42.301,94.297,94.297
						C495.805,410.423,485.968,434.074,468.105,451.937z"/>
					<path d="M401.509,64.67c-34.182,0-61.99,27.808-61.99,61.989h16.195c0-25.251,20.543-45.794,45.795-45.794
						c25.251,0,45.794,20.543,45.794,45.794c0,25.252-20.543,45.795-45.794,45.795v16.195c34.181,0,61.989-27.808,61.989-61.99
						C463.498,92.478,435.69,64.67,401.509,64.67z"/>
					<path d="M110.492,323.351c-34.182,0-61.99,27.808-61.99,61.99c0,34.181,27.808,61.989,61.99,61.989v-16.195
						c-25.252,0-45.795-20.543-45.795-45.794c0-25.252,20.543-45.795,45.795-45.795c25.251,0,45.794,20.543,45.794,45.795h16.195
						C172.481,351.158,144.674,323.351,110.492,323.351z"/>	
					</g>
					</svg>
				</span>
			</div>
			
			<div class="col-3 col-xl-1 mt-4 text-center">
				<span class="svg-icons" data-toggle="tooltip" data-placement="bottom" title="Development: increase your profit on account of partner offers or referral program.">
					<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
					viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
					<g>
						<path d="M488.399,492h-21.933V173.536c0-14.823-12.06-26.882-26.882-26.882H390.56c-14.823,0-26.882,12.06-26.882,26.882V492
							h-55.692V317.825c0-14.823-12.059-26.882-26.882-26.882H232.08c-14.823,0-26.882,12.06-26.882,26.882V492h-55.692v-90.204
							c0-14.823-12.06-26.882-26.882-26.882H73.599c-14.823,0-26.882,12.06-26.882,26.882V492H23.601c-5.523,0-10,4.477-10,10
							s4.477,10,10,10h464.798c5.523,0,10-4.477,10-10S493.922,492,488.399,492z M129.504,492H66.716v-90.204
							c0-3.795,3.087-6.882,6.882-6.882h49.024c3.795,0,6.882,3.087,6.882,6.882V492z M287.985,492h-62.788V317.825
							c0-3.795,3.087-6.882,6.882-6.882h49.024c3.794,0,6.882,3.087,6.882,6.882V492z M446.466,492h-62.788V173.536
							c0-3.795,3.087-6.882,6.882-6.882h49.024c3.795,0,6.882,3.087,6.882,6.882V492z"/>
						<path d="M466.442,10.516c0.14-2.729-0.82-5.504-2.904-7.588c-2.084-2.084-4.859-3.045-7.588-2.904
							C455.789,0.017,455.63,0,455.466,0h-60.5c-5.523,0-10,4.477-10,10s4.477,10,10,10h37.357l-98.857,98.858l-37.28-37.28
							c-1.875-1.875-4.419-2.929-7.071-2.929c-2.652,0-5.196,1.054-7.071,2.929l-179.769,179.77c-3.905,3.905-3.905,10.237,0,14.143
							c1.953,1.951,4.512,2.927,7.071,2.927s5.119-0.976,7.071-2.929L289.115,102.79l37.28,37.28c3.905,3.905,10.237,3.905,14.143,0
							L446.466,34.143v33.81c0,5.523,4.477,10,10,10s10-4.477,10-10V11C466.466,10.837,466.449,10.678,466.442,10.516z"/>
						<circle cx="75.64" cy="303.31" r="10"/>
					</g>
					</svg>
				</span>
			</div>
			
			<div class="col-3 col-xl-1 mt-4 text-center">
				<span class="svg-icons" data-toggle="tooltip" data-placement="bottom" title="Output crypto-current: earn more then you could earn from real mining!">
				<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
				viewBox="0 0 512.001 512.001" style="enable-background:new 0 0 512.001 512.001;" xml:space="preserve" >
					<g>
						<path d="M489.502,218.492h-7.496v-22.504v-80c0-26.192-21.309-47.501-47.5-47.501H228.527L89.564,28.783
							c-0.04-0.011-0.081-0.017-0.121-0.028c-0.041-0.011-0.08-0.027-0.121-0.037c-0.084-0.021-0.167-0.031-0.25-0.049
							c-0.129-0.027-0.258-0.055-0.387-0.076c-0.117-0.019-0.232-0.031-0.349-0.045c-0.136-0.016-0.271-0.03-0.406-0.038
							c-0.11-0.006-0.219-0.007-0.329-0.009c-0.14-0.002-0.279-0.003-0.418,0.003c-0.103,0.004-0.206,0.014-0.308,0.022
							c-0.143,0.012-0.286,0.025-0.427,0.046c-0.096,0.014-0.191,0.032-0.286,0.049c-0.146,0.026-0.29,0.054-0.433,0.089
							c-0.089,0.022-0.176,0.047-0.264,0.072c-0.146,0.042-0.29,0.085-0.434,0.135c-0.081,0.029-0.161,0.06-0.242,0.092
							c-0.145,0.056-0.289,0.115-0.43,0.18c-0.075,0.035-0.148,0.073-0.222,0.11c-0.141,0.071-0.28,0.143-0.416,0.224
							c-0.071,0.041-0.139,0.085-0.209,0.129c-0.134,0.084-0.266,0.17-0.394,0.263c-0.067,0.048-0.132,0.1-0.198,0.151
							c-0.123,0.095-0.245,0.192-0.362,0.295c-0.066,0.058-0.129,0.119-0.193,0.179c-0.11,0.104-0.219,0.208-0.323,0.318
							c-0.065,0.069-0.126,0.141-0.189,0.213c-0.095,0.109-0.189,0.219-0.278,0.334c-0.064,0.083-0.124,0.168-0.185,0.253
							c-0.079,0.111-0.157,0.223-0.229,0.338c-0.062,0.098-0.119,0.2-0.176,0.301c-0.063,0.111-0.126,0.223-0.183,0.338
							c-0.056,0.112-0.106,0.227-0.157,0.342c-0.05,0.113-0.099,0.226-0.143,0.342c-0.046,0.122-0.086,0.247-0.127,0.373
							c-0.026,0.081-0.058,0.157-0.082,0.239c-0.012,0.041-0.017,0.081-0.028,0.121c-0.011,0.041-0.027,0.08-0.037,0.121l-8.576,34.312
							H47.505c-26.139,0-47.411,21.223-47.497,47.343C0.007,115.882,0,115.933,0,115.986v260c0,4.14,3.37,7.5,7.5,7.5
							c4.14,0,7.5-3.36,7.5-7.5V150.579c8.504,7.996,19.938,12.91,32.505,12.91h387.001c0.009,0,0.017-0.001,0.026-0.001
							c17.908,0.014,32.474,14.588,32.474,32.501v52.504h-82.512c-0.005,0-0.009,0.001-0.014,0.001c-0.217,0-0.433,0.013-0.648,0.032
							c-0.239,0.021-0.475,0.055-0.707,0.098c-0.035,0.006-0.069,0.009-0.104,0.016c-0.196,0.039-0.388,0.091-0.579,0.145
							c-0.042,0.012-0.085,0.02-0.126,0.033c-0.17,0.052-0.335,0.114-0.5,0.177c-0.063,0.024-0.127,0.044-0.19,0.07
							c-0.138,0.057-0.271,0.124-0.404,0.189c-0.088,0.042-0.177,0.081-0.263,0.126c-0.104,0.055-0.202,0.118-0.302,0.178
							c-0.111,0.066-0.224,0.13-0.332,0.202c-0.073,0.048-0.14,0.103-0.21,0.154c-0.128,0.092-0.257,0.183-0.379,0.284
							c-0.053,0.044-0.102,0.093-0.153,0.138c-0.131,0.114-0.262,0.227-0.386,0.351c-0.073,0.073-0.139,0.153-0.208,0.229
							c-0.097,0.105-0.197,0.208-0.289,0.32c-0.155,0.189-0.3,0.385-0.436,0.588c-0.003,0.003-0.006,0.007-0.008,0.011l-39.998,60.001
							c-0.279,0.418-0.515,0.861-0.702,1.322c-0.071,0.172-0.128,0.347-0.185,0.521c-0.017,0.053-0.039,0.104-0.055,0.157
							c-0.056,0.185-0.099,0.372-0.14,0.558c-0.011,0.049-0.025,0.097-0.035,0.146c-0.033,0.168-0.055,0.337-0.076,0.506
							c-0.009,0.072-0.023,0.144-0.03,0.217c-0.014,0.139-0.017,0.277-0.022,0.416c-0.005,0.106-0.014,0.212-0.014,0.319
							c0,0.103,0.009,0.206,0.013,0.309c0.005,0.142,0.009,0.285,0.023,0.427c0.007,0.069,0.02,0.138,0.029,0.207
							c0.022,0.172,0.044,0.344,0.078,0.515c0.009,0.047,0.023,0.093,0.034,0.14c0.041,0.188,0.085,0.376,0.141,0.563
							c0.016,0.053,0.038,0.104,0.055,0.158c0.057,0.174,0.114,0.349,0.185,0.521c0.188,0.46,0.424,0.904,0.704,1.323l39.997,59.992
							c0.002,0.003,0.004,0.005,0.006,0.008c0.161,0.239,0.337,0.467,0.523,0.686c0.061,0.072,0.126,0.139,0.189,0.207
							c0.127,0.139,0.259,0.273,0.396,0.402c0.081,0.076,0.161,0.15,0.243,0.221c0.146,0.125,0.297,0.242,0.452,0.356
							c0.07,0.051,0.137,0.107,0.208,0.155c0.225,0.153,0.458,0.294,0.699,0.422c0.061,0.032,0.124,0.059,0.185,0.089
							c0.192,0.096,0.388,0.185,0.589,0.264c0.078,0.031,0.156,0.059,0.235,0.087c0.191,0.068,0.385,0.128,0.583,0.181
							c0.081,0.021,0.161,0.043,0.243,0.063c0.203,0.047,0.409,0.083,0.618,0.113c0.076,0.011,0.151,0.026,0.227,0.035
							c0.258,0.029,0.52,0.045,0.786,0.047c0.021,0,0.042,0.004,0.064,0.004c0.004,0,0.009-0.001,0.014-0.001h82.491V436
							c0,17.922-14.58,32.502-32.5,32.502h-387c-17.92,0-32.5-14.58-32.5-32.502c0-0.039-0.005-0.077-0.006-0.116V405.99
							c0-4.13-3.36-7.5-7.5-7.5c-4.13,0-7.5,3.37-7.5,7.5V436c0,0.078,0.009,0.154,0.012,0.231c0.125,26.086,21.38,47.271,47.494,47.271
							h387c26.189,0,47.496-21.305,47.5-47.494c0-0.001,0-0.002,0-0.004v-52.512h7.496c12.404,0,22.496-10.092,22.496-22.496V240.988
							C511.998,228.584,501.906,218.492,489.502,218.492z M92.9,45.336l132.339,37.813c0.006,0.002,0.013,0.003,0.019,0.005
							l88.696,25.342H77.114L92.9,45.336z M467.005,161.394c-8.503-7.994-19.935-12.907-32.499-12.907c-0.013,0-0.025,0.002-0.039,0.002
							H47.505c-17.921,0-32.501-14.58-32.501-32.501c0-17.921,14.58-32.501,32.501-32.501h20.398l-6.25,25.009H47.507
							c-4.142,0-7.5,3.358-7.5,7.5c0,4.142,3.358,7.5,7.5,7.5h19.976c0.01,0,0.021,0.002,0.032,0.002c0.008,0,0.016-0.002,0.023-0.002
							h299.941c0.01,0,0.019,0.002,0.028,0.002c0.008,0,0.016-0.002,0.024-0.002h56.966c4.142,0,7.5-3.358,7.5-7.5
							c0-4.142-3.358-7.5-7.5-7.5h-55.943l-87.529-25.009h153.48c17.92,0,32.5,14.58,32.5,32.501V161.394z M496.998,360.992
							L496.998,360.992c0,4.133-3.363,7.496-7.496,7.496H388.508l-34.999-52.494l34.999-52.502h100.994c0.105,0,0.207-0.012,0.311-0.016
							c2.516-0.035,4.932-0.483,7.185-1.281V360.992z M489.502,248.484c-0.052,0-0.103,0.007-0.155,0.008h-7.341v-15h7.496
							c4.133,0,7.496,3.363,7.496,7.496S493.635,248.484,489.502,248.484z"/>
						<path d="M414.499,288.493c-15.164,0-27.5,12.337-27.5,27.501s12.336,27.5,27.5,27.5s27.5-12.336,27.5-27.5
							C441.999,300.83,429.663,288.493,414.499,288.493z M414.499,328.494c-6.893,0-12.5-5.607-12.5-12.5s5.607-12.501,12.5-12.501
							s12.5,5.608,12.5,12.501S421.392,328.494,414.499,328.494z"/>
					</g>
				</svg>
				</span>
			</div>
		</div>
	</div>
	
	<div id="predlozh" class="container-fluid text-center">
		<span style="font-size:1.75rem;display:block;" class="mb-4 white">Valuable offers</span>
		
	  <div class="container">
		<div class="row">
			<div class="col-12 col-md-4">
				<a data-toggle="modal" data-target="#modal-miner" href="modal-miner">
					<div class="card mt-4">
						<img class="card-img-top" src="../img/mining.jpg" alt="Propose for miners">
						<div class="card-body">
							<p class="card-text">Reality in virtual reality. Earn on mining with minimal attachment and experience.</p>
						</div>
					</div>
				</a>
			</div>
			<div class="col-12 col-md-4">
				<a data-toggle="modal" data-target="#modal-gamer" href="modal-game">
					<div class="card mt-4">
						<img class="card-img-top" src="../img/game.jpg" alt="Propose for gamers">
						<div class="card-body">
							<p class="card-text">Play and get profit. Complete tasks, take a part in unique developments and increase your income.</p>
						</div>
					</div>
				</a>
			</div>
			<div class="col-12 col-md-4">
				<a data-toggle="modal" data-target="#modal-investor" href="modal-game">
					<div class="card mt-4">
						<img class="card-img-top" src="../img/business.jpg" alt="Propose for partners and investors">
						<div class="card-body">
							<p class="card-text">Become unique member of a platform. Use offers with increased profitability.</p>
						</div>
					</div>
				</a>
			</div>
		</div>
	  </div>
	</div>
	<!-- Modal Miner -->
	<div class="modal fade" id="modal-miner" tabindex="-1" role="dialog" aria-hidden="true">
	  <div class="modal-dialog" role="document">
		<div class="modal-content">
		  <div class="modal-header text-center">
			<span style="font-size:1.25rem;display:block;" class="modal-title red">Propose for miners</span>
			<button type="button" class="close" data-dismiss="modal" aria-label="Close">
			  <span aria-hidden="true">&times;</span>
			</button>
		  </div>
		  <div class="modal-body">
			<p><b>Pre-ICO</b></p>
			<p>Take a part in Bounty-program to get free tokens VM and decrease expenses for serve mining farm.</p>
			<p><b>ICO</b></p>
			<p>Buy VM tokens with discount. It would increase your income from mining.</p>
			<p><b>Game release</b></p>
			<p>Simple, accessible and interesting mining BTC in virtual reality.</p>
		  </div>
		  <div class="modal-footer">
			<button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
		  </div>
		</div>
	  </div>
	</div>
	<!-- Modal Gamer -->
	<div class="modal fade" id="modal-gamer" tabindex="-1" role="dialog" aria-hidden="true">
	  <div class="modal-dialog" role="document">
		<div class="modal-content">
		  <div class="modal-header text-center">
			<span style="font-size:1.25rem;display:block;" class="modal-title red">Propose for gamers</span>
			<button type="button" class="close" data-dismiss="modal" aria-label="Close">
			  <span aria-hidden="true">&times;</span>
			</button>
		  </div>
		  <div class="modal-body">
			<p><b>Pre-ICO</b></p>
			<p>Take a part in Bounty program allow's you to get free VM tokens.</p>
			<p><b>ICO</b></p>
			<p>Beta – testing for undistributed VM tokens.</p>
			<p><b>Game release</b></p>
			<p>Possibility to get unique equipment for BTC mining. Increase profit at the expense of partners offers.</p>
		  </div>
		  <div class="modal-footer">
			<button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
		  </div>
		</div>
	  </div>
	</div>
	<!-- Modal Investor -->
	<div class="modal fade" id="modal-investor" tabindex="-1" role="dialog" aria-hidden="true">
	  <div class="modal-dialog" role="document">
		<div class="modal-content">
		  <div class="modal-header text-center">
			<span style="font-size:1.25rem;display:block;" class="modal-title red">Propose for partners and investors</span>
			<button type="button" class="close" data-dismiss="modal" aria-label="Close">
			  <span aria-hidden="true">&times;</span>
			</button>
		  </div>
		  <div class="modal-body">
			<p>Now your services might be twice beneficial for users! We suggest you mutual advertising, new users and good opportunity to increase efficiency for your platform!</p>
			<p></p>
			<p>Our <b>investors</b> will get 100% of monopoly and will divide between themselves all expenses of miners on electricity, rent of premises and network traffic, which would increase with every new user.</p>
			<p></p>
		  </div>
		  <div class="modal-footer">
			<button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
		  </div>
		</div>
	  </div>
	</div>
	
	<div id="team" class="container-fluid">
        <span style="font-size:1.75rem;display:block;" class="text-center mb-5">Team</span>
        <div class="row justify-content-center no-gutters">
          <div class="col-12 col-sm-6 col-lg-2 text-center">
            <div class="testimonial-item mx-auto mb-5">
              <img class="photo img-fluid rounded-circle mb-3 shadow p-1" src="../img/alex.jpg" alt="Aleksandr Rachko - СVO Virtual Mining ">
              <span style="font-size:1.25rem;display:block;" class="red">Aleksandr Rachko</span>
              <p><em>CEO</em></p>
			  <p>
			  <a target="_blank" rel="nofollow" href="https://www.facebook.com/rachko.a"><i class="fa fa-facebook-official fa-fw"></i></a>
			  <a target="_blank" rel="nofollow" href="https://vk.com/arachko"><i class="fa fa-vk fa-fw"></i></a>
			  </p>
            </div>
          </div>
          <div class="col-12 col-sm-6 col-lg-2 text-center">
            <div class="testimonial-item mx-auto mb-5">
              <img class="photo img-fluid rounded-circle mb-3 shadow p-1" src="../img/max.jpg" alt="Maksim Belogurov - CTO Virtual Mining">
			  <span style="font-size:1.25rem;display:block;" class="red">Maksim Belogurov</span>
              <p><em>CTO</em></p>
			  <p>
			  <a target="_blank" rel="nofollow" href="https://www.facebook.com/maxbelogurov2018"><i class="fa fa-facebook-official fa-fw"></i></a>
			  <a target="_blank" rel="nofollow" href="https://vk.com/fin_anime"><i class="fa fa-vk fa-fw"></i></a>
			  <a target="_blank" rel="nofollow" href="https://www.instagram.com/blogmax.ru/"><i class="fa fa-instagram fa-fw"></i></a>
			  <a target="_blank" rel="nofollow" href="http://blogmax.ru"><i class="fa fa-globe fa-fw"></i></a>
			  </p>
            </div>
          </div>
          <div class="col-12 col-sm-6 col-lg-2 text-center">
            <div class="testimonial-item mx-auto mb-5">
              <img class="photo img-fluid rounded-circle mb-3 shadow p-1" src="../img/nic.jpg" alt="Nikolay Pitirimov - ICO менеджер Virtual Mining">
              <span style="font-size:1.25rem;display:block;" class="red">Nikolay Pitirimov</span>
              <p><em>ICO manager</em></p>
			  <p>
			  <a target="_blank" rel="nofollow" href="https://vk.com/pitnik"><i class="fa fa-vk fa-fw"></i></a>
			  </p>
            </div>
          </div>
		  <div class="col-12 col-sm-6 col-lg-2 text-center">
            <div class="testimonial-item mx-auto mb-5">
              <img class="photo img-fluid rounded-circle mb-3 shadow p-1" src="../img/vadim.jpg" alt="Vadim Lyapin - CMO Virtual Mining">
              <span style="font-size:1.25rem;display:block;" class="red">Vadim Lyapin</span>
              <p><em>CMO</em></p>
			  <p>
			  <a target="_blank" rel="nofollow" href="https://www.facebook.com/vadim.lyapin.71"><i class="fa fa-facebook-official fa-fw"></i></a>
			  <a target="_blank" rel="nofollow" href="https://vk.com/id2543085"><i class="fa fa-vk fa-fw"></i></a>
			  </p>
            </div>
          </div>
        </div>
		
		<div class="row justify-content-center no-gutters">
		  <div class="col-12 col-sm-6 col-lg-2 text-center">
            <div class="testimonial-item mx-auto mb-5 mb-lg-0">
              <img class="photo img-fluid rounded-circle mb-3 shadow p-1" src="../img/vladimir.jpg" alt="Vladimir Gonchar - ICO Advisor">
			  <span style="font-size:1.25rem;display:block;" class="red">Vladimir Gonchar</span>
              <p><em>ICO Advisor</em></p>
            </div>
          </div>
          <div class="col-12 col-sm-6 col-lg-2 text-center">
            <div class="testimonial-item mx-auto mb-5 mb-lg-0">
              <img class="photo img-fluid rounded-circle mb-3 shadow p-1" src="../img/leha.jpg" alt="Aleksey Gusev">
              <span style="font-size:1.25rem;display:block;" class="red">Aleksey Gusev</span>
              <p><em>Gameplay Advisor</em></p>
            </div>
          </div>
		  <div class="col-12 col-sm-6 col-lg-2 text-center">
            <div class="testimonial-item mx-auto mb-5 mb-lg-0">
              <img class="photo img-fluid rounded-circle mb-3 shadow p-1" src="../img/sandra.jpg" alt="Aleksandra Rychkova">
              <span style="font-size:1.25rem;display:block;" class="red">Aleksandra Rychkova</span>
              <p><em>Game designer</em></p>
            </div>
          </div>
		  <div class="col-12 col-sm-6 col-lg-2 text-center">
            <div class="testimonial-item mx-auto mb-5 mb-lg-0">
              <img class="photo img-fluid rounded-circle mb-3 shadow p-1" src="../img/test.jpg" alt="Virtual Mining">
			  <span style="font-size:1.25rem;display:block;" class="red">...</span>
              <p><em>Position</em></p>
            </div>
          </div>
        </div>
    </div>
	
	<div id="copyright" class="container-fluid">
		<div class="row justify-content-center no-gutters footer-social">
          <div class="col-lg-12 text-center">
            <ul class="list-inline">
			  <li class="list-inline-item">
                <a target="_blank" rel="nofollow" href="https://www.facebook.com/virtualminingworld/">
                  <i class="fa fa-facebook-official fa-fw fa-2x"></i>
                </a>
              </li>
			  <li class="list-inline-item">
                <a target="_blank" rel="nofollow" href="#">
                  <i class="fa fa-vk fa-fw fa-2x"></i>
                </a>
              </li>
			  <li class="list-inline-item">
                <a target="_blank" rel="nofollow" href="https://www.instagram.com/virtualminingworld/">
                  <i class="fa fa-instagram fa-fw fa-2x"></i>
                </a>
              </li>
              <li class="list-inline-item">
                <a target="_blank" rel="nofollow" href="#">
                  <i class="fa fa-telegram fa-fw fa-2x"></i>
                </a>
              </li> 
            </ul>
          </div>
        </div>
		<div class="row text-center no-gutters">
			<div class="col-12">
				<p>© 2018 vmtoken.io</p>
			</div>
		</div>
	</div>
	


    <!-- Optional JavaScript -->
	
    <!-- jQuery first, then Popper.js, then Bootstrap JS -->
	<script src="../js/jquery.min.js"></script>
    <script src="../js/bootstrap.min.js"></script>
	<script src="../js/bootstrap.bundle.min.js"></script>
	
	<script>
		$(function () {
		$('[data-toggle="tooltip"]').tooltip()
		})
	</script>
	
	<!-- Yandex.Metrika counter -->
		<script>
			(function (d, w, c) {
				(w[c] = w[c] || []).push(function() {
					try {
						w.yaCounter49588912 = new Ya.Metrika2({
							id:49588912,
							clickmap:true,
							trackLinks:true,
							accurateTrackBounce:true,
							webvisor:true
						});
					} catch(e) { }
				});

				var n = d.getElementsByTagName("script")[0],
					s = d.createElement("script"),
					f = function () { n.parentNode.insertBefore(s, n); };
				s.type = "text/javascript";
				s.async = true;
				s.src = "//mc.yandex.ru/metrika/tag.js";

				if (w.opera == "[object Opera]") {
					d.addEventListener("DOMContentLoaded", f, false);
				} else { f(); }
			})(document, window, "yandex_metrika_callbacks2");
		</script>
		<!--<noscript><div><img src="https://mc.yandex.ru/watch/49588912" style="position:absolute; left:-9999px;" alt="" /></div></noscript>-->
	<!-- /Yandex.Metrika counter -->
  </body>
</html>